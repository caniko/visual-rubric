...
fn main() {
    let mode = std::env::var("FAKE_OPENCODE_MODE").unwrap_or_else(|_| "quota".to_string());
    if mode == "crash" {
        eprintln!("fake opencode crash requested");
        std::process::exit(2);
    }
    if let Ok(path) = std::env::var("FAKE_OPENCODE_SPAWN_LOG") {
    ...
    if let Ok(path) = std::env::var("FAKE_OPENCODE_ARG_LOG") {
        let _ = OpenOptions::new()
            .create(true)
            .append(true)
            .open(path)
    ...
    if let Ok(path) = std::env::var("FAKE_OPENCODE_CWD_LOG") {
    ...
    if let Ok(path) = std::env::var("FAKE_OPENCODE_ENV_LOG") {
        let value = std::env::var("FAKE_OPENCODE_LOG_ENV_KEY")
            .ok()
            .and_then(|key| std::env::var(key).ok())
            .or_else(|| std::env::var("FAKE_OPENCODE_CUSTOM_ENV").ok());
    ...
    for line in BufReader::new(stdin.lock()).lines() {
        ...
        let response = match method {
            ...
            "session/prompt" if mode == "prompt_crash" => {
                eprintln!("fake opencode prompt crash requested");
                std::process::exit(3);
            }
            "session/prompt"
                if mode == "pass" || mode == "fail" || mode == "chunks" || mode == "malformed" =>
            {
                ...
                if let Ok(path) = std::env::var("FAKE_OPENCODE_PROMPT_LOG") {
